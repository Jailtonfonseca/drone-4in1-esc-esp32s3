...
v3: MOSFETs em rot 90 (fluxo vertical), 4 colunas (1 por motor) x 3 fases.
...
OUT = "/opt/jupyter/work/drone/fase3_pcb/v3"
...
for mi, cx in enumerate(COL_X):
    for ph, y in enumerate(ROW_Y):
        ...
        place("Resistor_SMD.pretty", "R_2512_6332Metric", "Rs%s" % t, cx + 12, y + 28, 90)
        via(cx + 12, y + 1, "VBAT_PROT"); via(cx + 12, y + 33, "GND")
        # --- controle ---
        ...
        wire("Q%sH" % t, "3", "Q%sL" % t, "2", "PH%s" % t, W_PHASE)   # source HS -> dreno LS
        wire("Q%sL" % t, "3", "Rs%s" % t, "1", "SN%s" % t, W_PHASE)   # source LS -> shunt
        wire_pt("Rs%s" % t, "2", cx + 12, y + 33, "GND", W_PHASE)     # shunt -> via GND
...
BRD.Save(OUT + "/v3_drone.kicad_pcb")
print("salvou", OUT + "/v3_drone.kicad_pcb")
