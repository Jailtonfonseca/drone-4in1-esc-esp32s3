...
v2: grade de espacamento real (v1 colidia). Nao sobrescreve a v1.
Roda: /usr/bin/python3.9 gera_pcb_v2.py
"""
import os
import pcbnew

LIB = "/usr/share/kicad/modules/"
OUT = "/opt/jupyter/work/drone/fase3_pcb/v2"
...
X0, Y0, W, H = 10.0, 10.0, 200.0, 160.0
...
def zone(netname, layer, pts):
    ...
    z.SetNetCode(net(netname).GetNet()); o = z.Outline(); o.NewOutline()
    for x, y in pts:
        o.Append(pcbnew.FromMM(x), pcbnew.FromMM(y))
    BRD.Add(z); return z

# larguras calculadas (calc_trilhas_vias.py, IPC-2221, 2 oz, dT 10 C)
...
def make_smd_pad(mod, name, lx, ly, sx, sy, ox, oy, shape=pcbnew.PAD_SHAPE_RECT):
    ...
    for lay in (pcbnew.F_Cu, pcbnew.F_Mask, pcbnew.F_Paste):
        ls.AddLayer(lay)
    p.SetLayerSet(ls)
    p.SetPos0(pcbnew.wxPointMM(lx, ly))                  # local (relativo ao modulo)
    p.SetPosition(pcbnew.wxPointMM(ox + lx, oy + ly))    # absoluto
    mod.Add(p)

def make_module(ref, ox, oy, n_pins=41, pitch=1.27, bw=25.5, bh=18.0):
    """ESP32-S3-WROOM-1 (geometria do modulo; MAPEAMENTO pad->sinal NAO
    verificado -- datasheet offline). Pads nomeados 1..41."""
...
# ================================================================
# 12 MEIAS-PONTES: (motor 1..4) x (fase 1..3)
# ================================================================
MOT = [(74.0, 50.0), (126.0, 50.0), (74.0, 102.0), (126.0, 102.0)]
PITCH_PHASE = 16.0
route_log = []

for mi, (bx, yA) in enumerate(MOT):
    for ph in range(3):
        y = yA + ph * PITCH_PHASE
        t = "M%d%02d" % (mi + 1, ph + 1)
        place("Package_TO_SOT_SMD.pretty", "TO-252-3_TabPin2", "Q%sH" % t, bx + 16, y)
        place("Package_TO_SOT_SMD.pretty", "TO-252-3_TabPin2", "Q%sL" % t, bx + 32, y)
        place("Resistor_SMD.pretty", "R_2512_6332Metric", "Rs%s" % t, bx + 43, y)
        place("Package_SO.pretty", "SOIC-8_3.9x4.9mm_P1.27mm", "U%s" % t, bx + 4, y + 8)
        place("Resistor_SMD.pretty", "R_0805_2012Metric", "Rgo%s" % t, bx + 11, y + 7)
        place("Resistor_SMD.pretty", "R_0805_2012Metric", "Rgf%s" % t, bx + 11, y + 10)
        place("Diode_SMD.pretty", "D_SOD-123", "Db%s" % t, bx + 2, y + 14)
        place("Capacitor_SMD.pretty", "C_0805_2012Metric", "Cb%s" % t, bx + 6, y + 14)
        place("Capacitor_SMD.pretty", "C_1206_3216Metric", "Cv%s" % t, bx + 10, y + 14)
        place("Capacitor_SMD.pretty", "C_0402_1005Metric", "Ca%s" % t, bx + 14, y + 14)
        place("Package_SO.pretty", "SOIC-8_3.9x4.9mm_P1.27mm", "Ua%s" % t, bx + 40, y + 8)
        place("Resistor_SMD.pretty", "R_0603_1608Metric", "Rd1%s" % t, bx + 47, y + 6)
        place("Resistor_SMD.pretty", "R_0603_1608Metric", "Rd2%s" % t, bx + 47, y + 9)
        place("Capacitor_SMD.pretty", "C_0402_1005Metric", "Cd%s" % t, bx + 47, y + 12)
        place("Resistor_SMD.pretty", "R_0603_1608Metric", "Rb1%s" % t, bx + 36, y + 14)
        place("Resistor_SMD.pretty", "R_0603_1608Metric", "Rb2%s" % t, bx + 39, y + 14)
        place("Capacitor_SMD.pretty", "C_0402_1005Metric", "Cbe%s" % t, bx + 42, y + 14)

        # ---- netlist / roteamento defensavel (independe do pinout do driver) ----
        setnet("Q%sH" % t, "2", "VBAT")
        setnet("Q%sL" % t, "2", "PH%s" % t)
        setnet("Q%sH" % t, "3", "PH%s" % t)
        setnet("Q%sL" % t, "3", "SN%s" % t)
        setnet("Rs%s" % t, "1", "SN%s" % t)
        setnet("Rs%s" % t, "2", "GND")
        setnet("Cb%s" % t, "2", "PH%s" % t)
        setnet("Cb%s" % t, "1", "VB%s" % t)      # nó do bootstrap (pino do driver NAO verificado)
        setnet("Db%s" % t, "2", "VB%s" % t)
        setnet("Db%s" % t, "1", "12V")
        setnet("Cv%s" % t, "1", "12V"); setnet("Cv%s" % t, "2", "GND")
        setnet("Ca%s" % t, "1", "VBAT"); setnet("Ca%s" % t, "2", "GND")
        setnet("Q%sH" % t, "1", "GH%s" % t)
        setnet("Q%sL" % t, "1", "GL%s" % t)
        setnet("Rgo%s" % t, "2", "GH%s" % t)
        setnet("Rgf%s" % t, "2", "GH%s" % t)
        setnet("Rgo%s" % t, "1", "DRO%s" % t)     # saida do driver (pinout NAO verificado)
        setnet("Rgf%s" % t, "1", "DRF%s" % t)
        ...
        for r in ("Rd1%s" % t, "Rd2%s" % t):
            setnet(r, "1", "VREF")
        setnet("Cd%s" % t, "1", "VREF"); setnet("Cd%s" % t, "2", "GND")
        setnet("Rb1%s" % t, "1", "PH%s" % t)
        setnet("Rb2%s" % t, "2", "GND")
        setnet("Cbe%s" % t, "1", "RB%s" % t); setnet("Cbe%s" % t, "2", "GND")

        # ---- trilhas de potencia (largura calculada) ----
        wire("Q%sH" % t, "3", "Q%sL" % t, "2", "PH%s" % t, W_PHASE)      # no da fase
        wire("Q%sL" % t, "3", "Rs%s" % t, "1", "SN%s" % t, W_PHASE)      # source LS -> shunt
        wire_pt("Rs%s" % t, "2", bx + 49, y, "GND", W_PHASE)             # shunt -> via de GND
        via(bx + 49, y, "GND")
        wire_pt("Q%sH" % t, "2", bx + 8, y, "VBAT", W_VBAT)              # stub do dreno HS
        via(bx + 8, y, "VBAT")

        # ---- gate (largura de sinal) ----
        wire("Q%sH" % t, "1", "Rgo%s" % t, "2", "GH%s" % t, W_GATE)
        wire_pt("Rgf%s" % t, "2", bx + 19, y + 3, "GH%s" % t, W_GATE)
        wire_pt("Q%sL" % t, "1", bx + 26, y + 5, "GL%s" % t, W_GATE)
        wire_pt("Rgf%s" % t, "1", bx + 14, y + 5, "DRF%s" % t, W_GATE)
        route_log.append(t)
...
# ================================================================
place("Connector_AMASS.pretty", "AMASS_XT60-F_1x02_P7.20mm_Vertical", "J1", 30, 142)
setnet("J1", "1", "VBAT"); setnet("J1", "2", "GND")
place("Package_TO_SOT_SMD.pretty", "TO-252-2_TabPin1", "Q1", 46, 124)
setnet("Q1", "1", "VBAT"); setnet("Q1", "2", "VBAT_PROT")
place("Diode_SMD.pretty", "D_SMB", "D1", 24, 124)
setnet("D1", "1", "VBAT_PROT"); setnet("D1", "2", "GND")
for i in range(6):
    r, c = divmod(i, 2)
    x = 20 + r * 14
    yy = 92 + c * 12
    place("Capacitor_SMD.pretty", "CP_Elec_8x10", "Cblk%d" % (i + 1), x, yy)
...
buck("B1", 46, "12V"); buck("B2", 58, "5V"); buck("B3", 70, "3V3")
place("Package_TO_SOT_SMD.pretty", "SOT-23", "U_LDO", 58, 78)
...
# MCU / SENSORES / USB / UI
# ================================================================
m = make_module("U_MCU", 40, 158)
for p in m.Pads():
    p.SetNetCode(net("MCU_%s" % p.GetPadName()).GetNet())
place("Sensor_Motion.pretty", "InvenSense_QFN-24_3x3mm_P0.4mm", "U_IMU", 22, 152)
for p in MODS["U_IMU"].Pads():
    p.SetNetCode(net("IMU_%s" % p.GetPadName()).GetNet())
place("Package_LGA.pretty", "Bosch_LGA-8_2x2.5mm_P0.65mm_ClockwisePinNumbering", "U_BARO", 58, 154)
...
place("LED_SMD.pretty", "LED_0805_2012Metric", "D_LED", 16, 146)
setnet("D_LED", "2", "3V3"); setnet("D_LED", "1", "LED_K")
place("Resistor_SMD.pretty", "R_0603_1608Metric", "Rled", 16, 150)
setnet("Rled", "1", "LED_K"); setnet("Rled", "2", "GND")
place("Button_Switch_SMD.pretty", "SW_Push_1P1T_NO_CK_KMR2", "SW1", 16, 156)
place("Button_Switch_SMD.pretty", "SW_Push_1P1T_NO_CK_KMR2", "SW2", 16, 162)

# ================================================================
# CONECTORES DE MOTOR (1 por bloco)
# ================================================================
MCONN = [(95, 24), (145, 24), (95, 160), (145, 160)]
for mi, (x, y) in enumerate(MCONN):
    place("Connector_AMASS.pretty", "AMASS_MR30PW-FB_1x03_P3.50mm_Horizontal", "J_M%d" % (mi + 1), x, y)
    for ph in range(3):
...
zone("VBAT_PROT", pcbnew.In2_Cu, [(X0 + 2, Y0 + 2), (X0 + 60, Y0 + 2), (X0 + 60, Y0 + H - 2), (X0 + 2, Y0 + H - 2)])

n_via = 0
for yv in range(14, 166, 7):
    for xv in range(13, 208, 7):
        via(xv, yv, "GND"); n_via += 1
print("vias de costura GND:", n_via, "| clusters roteados:", len(route_log))

pcbnew.ZONE_FILLER(BRD).Fill(BRD.Zones())
BRD.Save(OUT + "/v2_drone.kicad_pcb")
print("salvou", OUT + "/v2_drone.kicad_pcb")
