...
p("=" * 78)
p("FASE 0 -- LIMITES CRITICOS (v4, final)")
p("=" * 78)
...
p("      Isso reduz a corrente RMS no banco de capacitores 4,5x (medido neste modelo) e e' de graca (e' configuracao de timer).")
...
for nome, x in [("cap dedicado por motor (adotado)", ib - ib.mean())]:
    for C, esr in [(470e-6, 15e-3)]:
        ...
        p(f"    {nome}: dV_C={(v.max()-v.min())*1e3:.0f} mV + dV_ESR={np.abs(x).max()*esr*1e3:.0f} mV "
          ...
          f"({((v.max()-v.min())*1e3+np.abs(x).max()*esr*1e3)/25200*100:.1f} % de 25.2 V)")
...
p("="*78)
open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "verifica_limites_saida_v4.txt"),"w").write("\n".join(L)+"\n")
